...
function Demo() {
  ...
  if (!USER_ID) {
    return (
      <div className="container text-center mt-5">
        ...
        <p>
          You must have an active user to use this API.
          Please create one using the API, see the docs at{' '}
          <a
  ...
  const handleAddTask = async () => {
    ...
    try {
      ...
      const newTask = await addTodo({ title: task, completed: false, user_id: USER_ID });
      // make a request to modify the store
      dispatch({ type: 'SET_TODOS', payload: { todos: [...store.todos, newTask] } });
      // clear the input
      setTask('');
    } catch (error) {
      console.error(error);
  ...
  const handleDeleteTask = async (id) => {
    try {
      ...
      // make a request to modify the store
      dispatch({ type: 'SET_TODOS', payload: { todos: store.todos.filter(task => task.id !== id) } });
    } catch (error) {
      console.error(error);
    }
  };
        ...
        <ul className="list-unstyled list-group ">
          {store.todos.map((todo) => (
            <li className="list-group-item d-flex justify-content-between" key={todo.id}>
              <div>{todo.title}</div>
              <button onClick={() => handleDeleteTask(todo.id)} className="btn border border-0 m-0 p-0 fw-bold text-danger">X</button>
            </li>
          ))}
        </ul>
      </div>
